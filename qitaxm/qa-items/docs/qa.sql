...
CREATE TABLE `prestige_flow` (
    `id`         INT(10)   NOT NULL AUTO_INCREMENT,
    `user_id`    INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建者ID',
    `prestige`     INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '变动后声望值',
    `amount`     INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '变动的声望值',
    `type`       INT(10)  NOT NULL DEFAULT 0 COMMENT '变动类型',
    `link_id`    INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '关联的数据表ID',
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB DEFAULT CHARSET = `utf8mb4` COMMENT ='声望变更历史表';